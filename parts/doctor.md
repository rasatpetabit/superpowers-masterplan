...
| 39 | **Codex auth expired or stale** (repo-scoped, v5.1.1+, I-1 of cosmic-cuddling-dusk; v5.2.3 refinement: chatgpt-mode cosmetic-expiry skip). Reads `~/.codex/auth.json`. Decodes JWT `exp` claim from `.tokens.id_token` and `.tokens.access_token` (with fallback to top-level `.id_token`/`.access_token` for older schemas). Fires on: (a) either token expired (`now > exp`); (b) either token expires within 24h (`exp - now < 86400`); (c) `last_refresh` > 30 days ago even when tokens are within validity. **Skipped (PASS with INFO) when `auth_mode == "chatgpt"` AND `tokens.refresh_token` is present AND `last_refresh` is within the last 7 days** — under chatgpt mode the short-lived JWTs auto-refresh via the persistent `refresh_token` on every call, so a past-due `exp` is the steady-state norm. Diagnoses the upstream cause of Codex routing/review silently degrading to off — Step 0's ping returns an error, the framework correctly applies `degrade-loudly`, but the user has no idea WHY. Pairs with check #18 (config-vs-plugin mismatch): #18 flags persistent misconfig; #39 flags expired credentials. Skipped silently when `~/.codex/auth.json` is absent (codex not installed). | Warning | Report per-token expiry timestamp + age in days. Suggest `codex login` (or equivalent shell-based refresh — varies by codex CLI version). No auto-fix (auth refresh is browser-based OAuth, user-owned per headless-host constraint). |
...
if [ ! -r "$auth" ]; then
  ...
  now="$(date +%s)"
  # v5.2.3: chatgpt-mode cosmetic-expiry skip.
  # Under auth_mode=chatgpt the JWTs are short-lived (~1h) and auto-refreshed
  # via the persistent refresh_token on every Codex call. A past-due exp is
  # the steady-state norm, NOT a degradation signal. Skip sub-fires (a) and (b)
  # when the chatgpt-healthy shape holds; keep sub-fire (c) (last_refresh > 30d)
  # active since stale refresh IS evidence of real trouble even in chatgpt mode.
  ...
  if [ "$auth_mode" = "chatgpt" ] && [ -n "$refresh_token" ] && [ -n "$last_refresh" ]; then
    ...
    if [ "$refresh_sec" -gt 0 ]; then
      ...
      if [ $refresh_age_days -le 7 ]; then
        cosmetic_skip=1
        echo "Check #39: PASS (auth_mode=chatgpt; JWT auto-refresh healthy; last_refresh ${refresh_age_days}d ago)"
      fi
    fi
  fi
  if [ $cosmetic_skip -eq 0 ]; then
...
if [ -r "$auth" ]; then
  now="$(date +%s)"
  # v5.2.3: under auth_mode=chatgpt with refresh_token + recent last_refresh,
  # treat auth as healthy regardless of JWT exp (short-lived JWTs auto-refresh
  # on every Codex call via the persistent refresh_token).
  auth_mode="$(jq -r '.auth_mode // empty' "$auth" 2>/dev/null)"
  refresh_token="$(jq -r '.tokens.refresh_token // .refresh_token // empty' "$auth" 2>/dev/null)"
  last_refresh="$(jq -r '.last_refresh // empty' "$auth" 2>/dev/null)"
  if [ "$auth_mode" = "chatgpt" ] && [ -n "$refresh_token" ] && [ -n "$last_refresh" ]; then
    ...
    if [ "$refresh_sec" -gt 0 ]; then
      refresh_age_days=$(( (now - refresh_sec) / 86400 ))
      [ $refresh_age_days -le 7 ] && auth_healthy=1
    fi
  fi
  if [ $auth_healthy -eq 0 ]; then
    for field in id_token access_token; do
      token="$(jq -r ".tokens.$field // .$field // empty" "$auth" 2>/dev/null)"
      [ -z "$token" ] && continue
      payload="$(echo "$token" | cut -d. -f2)"
