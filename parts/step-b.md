...
> Default every single-file task to `**Codex:** ok`. This applies to code edits AND doc edits — Codex is competent at markdown. Only mark `**Codex:** no` when ANY of these apply: (a) multi-file edit, (b) ambiguous scope, (c) no known verification command, (d) explicit scope-out from the user. Keep the per-task `**Codex:** no` escape hatch for tasks that fail this rule. See the Plan annotations subsection in Step C 3a for the exact syntax. The orchestrator's eligibility cache parses these as overrides on the heuristic checklist.
...
>
> - complexity == low — brief writing-plans to: produce a flat task list of ~3–7 tasks; apply the single-file `**Codex:** ok` default when emitting annotations; SKIP the `**parallel-group:**` annotation guidance; mark `**Files:**` blocks as OPTIONAL (best-effort, not required). Plan output is leaner.
> - `complexity == medium` — current brief (above bullets are the canonical defaults; `**Files:**` encouraged, single-file tasks default to `**Codex:** ok`, `**parallel-group:**` optional). No change.
> - `complexity == high` — brief writing-plans to: REQUIRE `**Files:**` block per task (exhaustive); REQUIRE `**Codex:**` annotation per task (`ok` for single-file tasks unless the documented `no` escape hatch applies, otherwise `no`); ENCOURAGE `**parallel-group:**` for verification/lint/inference clusters. Eligibility cache will be validated against `**Files:**` declarations at Step C step 1 (per spec §Behavior matrix / Plan-writing / `eligibility cache` row at high). Because every task carries a well-formed annotation pair by construction, Step C step 1's Build path always takes the inline fast-path at `high` (no Haiku dispatch); see **Inline-build verifier** in Step C step 1.
