...
- **Codex-routing visibility (v5.1.1+, checks #39–#41):** three read-only diagnostics that surface the silent-degradation paths discovered during cosmic-cuddling-dusk investigation. **#39** (`codex_auth_expiry`, repo-scoped): reads `~/.codex/auth.json`, base64url-decodes the JWT `exp` claim from `.tokens.id_token` and `.tokens.access_token` (with fallback to top-level for older schemas), warns when either is expired/expiring or `last_refresh` is > 30 days; skipped silently when codex is not installed. **v5.2.3 refinement:** sub-fires (a)/(b) (expired or near-expiry tokens) are skipped when `auth_mode == "chatgpt"` AND `tokens.refresh_token` is present AND `last_refresh` is within 7 days — under chatgpt mode the short-lived JWTs auto-refresh on every call, so a past-due `exp` is the steady-state norm rather than a degradation signal. Step 0's boot-banner Codex health line applies the same cosmetic-shape skip. The `codex_health_check_jwt_only` watcher added in v5.2.1 to surface this false-positive class was retired in v5.2.3 once the underlying check was fixed. Pairs with #18 (config-vs-plugin mismatch). **#40** (`high_complexity_codex_annotation_gap`, plan-scoped): for `complexity: high` plans, compares `^### Task ` heading count against `**Codex:** (ok|no)` annotation count; INFO-flags zero `**parallel-group:**` annotations. Catches the writing-plans skill silently skipping the high-complexity brief — which suppresses Codex eligibility caching and parallel-wave assembly. **#41** (`missing_codex_degradation_evidence`, plan-scoped): two sub-fires — (a) routing/review forced off with healthy auth and no `codex degraded` event in `events.jsonl` (degrade-loudly visibility violation); (b) routing on with `codex_ping ok` events but zero `routing→[codex]` events (every task ineligible — cross-references #40). All three report-only.
