...
**Parallelization.** When worktrees ≥ 2, dispatch one Haiku agent (pass `model: "haiku"` per §Agent dispatch contract) per worktree in a single Agent batch (each agent runs all 24 plan-scoped checks for its worktree and returns findings as `[{check_id, severity, file, message}]` JSON). With 1 worktree, run inline — agent dispatch latency isn't worth it. The orchestrator merges results and applies the report ordering below. Repo-scoped checks #25 (self-host deployment drift, v2.9.0+), #26 (`auto_compact_loop_attached`, v2.9.1+), and #27 (`orchestrator_free_text_user_question`, v2.10.0+) fire ONCE per doctor run regardless of worktree/plan count and run inline at the orchestrator. Their inputs are session-level or repo-source state (user-level paths + repo HEAD for #25; `CronList` output for #26; `commands/masterplan.md` source grep for #27), not per-plan state. The plan-scoped check count (24) is unchanged.
...
- `medium` plans: run all 24 plan-scoped checks except #22 (high-only).
- `high` plans: run all 24 plan-scoped checks INCLUDING #22 (high-complexity rigor evidence).
- Plans without a `complexity:` frontmatter field: treat as `medium`.

The check-set gate is per-plan: a single `/masterplan doctor` run against worktrees containing a mix of low/medium/high plans honors each plan's complexity individually. Findings are reported with the same severity as today. Check #25 (self-host deployment drift, v2.9.0+) is **repo-scoped** — it fires once per doctor run independent of any plan's complexity, only when `git config --get remote.origin.url` matches `superpowers-masterplan`.
...
| 27 | **`orchestrator_free_text_user_question`** (v2.10.0+; **repo-scoped** — fires once per doctor run, NOT per plan). Skipped silently when `git config --get remote.origin.url` does not match `superpowers-masterplan` (this check only fires inside this repo). Greps `commands/masterplan.md` for forbidden free-text question patterns: `ask the user`, `prompt the user`, `request that the user`, `request the user`, `confirm with the user`, `wait for the user('s response\| to)`, `the user must (confirm\|choose\|decide\|pick)`. For each match, scans ±20 lines for a paired `AskUserQuestion` call OR an inline exemption marker `<!-- cd9-exempt: <reason> -->`. Skips matches inside the rule definitions themselves (the CD-9 definition block and the "Don't stop silently" operational restatement) — these describe the rule, not violate it; detect by checking whether the matched line is inside a `**CD-9**` block or a `Don't stop silently` block within a 5-line window. Each unmatched/un-exempted occurrence emits Warning with file:line, matched text, and remediation guidance. Implements the v2.10.0 design goal #4 ("Structured questions, never free-text") and protects against future CD-9 regressions in the orchestrator. | Warning | No `--fix` available — rewriting prose to a structured question is a judgment call. Report each finding as: `⚠️ orchestrator_free_text_user_question — file:line — "<matched-text>" — no AskUserQuestion paired within ±20 lines and no cd9-exempt marker. Replace with AskUserQuestion or annotate with \`<!-- cd9-exempt: <reason> -->\`.` |
