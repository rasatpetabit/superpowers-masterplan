...
|---|---|---|---|
| 1 | **Legacy plan not migrated** — pre-v3 plan/spec/status/retro exists under `docs/superpowers/...`, is not referenced by any bundle `state.yml` `artifacts.*` or `legacy.*` path in the same worktree, and has no matching `docs/masterplan/<slug>/state.yml`. | Warning | `--fix`: run `bin/masterplan-state.sh migrate --write --slug=<slug>` (copy-only; no legacy delete). |
...
#   linked_at: 2026-05-01T14:32:00Z
